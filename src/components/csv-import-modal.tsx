...
export default function CSVImportModal({ isOpen, onClose, onImportComplete }: CSVImportModalProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      // Re-read the file to get all rows
      ...
      for (let i = 1; i < lines.length; i += batchSize) {
        ...
        for (const line of batch) {
          ...
          if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
            ...
            importResults.errors.push(`Row ${i + 1}: Invalid or missing email`)
            continue
          }
        ...
        if (contactsToInsert.length > 0) {
          try {
            ...
            importResults.errors.push(`Batch error: ${err.message}`)
            importResults.skipped += contactsToInsert.length
          }
        }

        importResults.total = lines.length - 1
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-10 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-medium text-gray-900">Import Contacts from CSV</h3>
